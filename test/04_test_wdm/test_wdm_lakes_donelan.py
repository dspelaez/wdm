...
frequencies, coefficients = wavelet_spectrogram(A, fs, omin, omax, nvoice, mode='BC')

# compute components of wavenumber
k, l = klcomponents(frequencies, coefficients, x, y, limit=1.5*np.pi, mask=np.nan)
# k, l = 1.2*k, 1.2*l
...
K_donelan = K_donelan / kfac / (fs/2)
K_donelan = smooth(K_donelan, ws=(15,3))
...
ax1.loglog(frequencies, power_wavelet.mean(2).mean(1)/delta_frequencies, 'b', label='Wavelet2')
...
ax2 = fig.add_subplot(1,2,2)
#
ax1.loglog(wavenumbers, np.trapz(K_donelan, x=dirs*np.pi/180., axis=0), 'g')
ax1.loglog(wavenumbers[wavenumbers > 1], 1E-2*wavenumbers[wavenumbers > 1]**-4)
# ax1.text(1.1, .01, '$f^{-5}$')
# ax1.legend(loc=0)
#
polar_spectrum(wavenumbers, dirs, K_donelan, fmax=4, ax=ax2, cbar=False, 
